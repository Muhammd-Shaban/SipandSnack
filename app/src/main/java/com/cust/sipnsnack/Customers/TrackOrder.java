...
public class TrackOrder extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        location = CustomerOTWAdapter.address;
        phone = CustomerOTWAdapter.phoneNo;
        orderId = CustomerOTWAdapter.orderID;

        if(location != null || !location.equals(""))
        {
            Geocoder geocoder = new Geocoder(TrackOrder.this);
            try
            {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if(location != null || !location.equals(""))
                {
                    ...
                    if (addressList.size() == 0) {
                        Toast.makeText(TrackOrder.this, "Your Provided Address is Invalid ...\nYour Location can't be Marked !!", Toast.LENGTH_LONG).show();
                    } else {
                        int height = 85;
                        int width = 85;
