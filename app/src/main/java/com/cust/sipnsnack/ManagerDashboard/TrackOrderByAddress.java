...
public class TrackOrderByAddress extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_track_order_by_address);
        ...
        {
            Geocoder geocoder = new Geocoder(TrackOrderByAddress.this);
            try
            {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                if(location != null || !location.equals(""))
                {
                    ...
                    if (addressList.size() == 0) {
                        Toast.makeText(TrackOrderByAddress.this, "Customer Provided Address is Invalid ...\nCustomer's Location can't be Marked !!", Toast.LENGTH_LONG).show();
                    } else {
                        int height = 85;
                        int width = 85;
                        ...
                        Address address = addressList.get(0);
                        customerLocation = new LatLng(address.getLatitude(),address.getLongitude());
                        map.addMarker(new MarkerOptions().position(customerLocation).title("Customer Location").anchor(0.5f, 0.5f)
